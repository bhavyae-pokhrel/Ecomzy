...
// START
import { BuyItem } from "../Payment/BuyOrder"; 
...
const Cart = () => {
  ...
  console.log('token in cart.jsx',token)

  
  const handleBuyOrder = () => {    
    if(token){
      BuyItem(token,cart, navigate, dispatch) 
    }
    else{
      console.log('token in handleBuyOrder',token)
    ...
    (<div className="flex gap-16 max-w-6xl p-6 mx-auto flex-wrap lg:flex-nowrap">
      ...
      <div className="md:w-[30%] w-full flex flex-col gap-8 justify-between">
        ...
        <div className="mb-20">
          ...
          border-2 border-[#15803d] hover:bg-white hover:text-[#15803d] transition-all duration-300 ease-in" onClick={handleBuyOrder} >
            CheckOut Now
          </button>
        </div>
