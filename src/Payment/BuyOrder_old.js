...
export async function BuyItem(token, cart, navigate, dispatch) {
  ...
  try {
    ...
    console.log("Razorpay key loaded:", !!razorpayKey, razorpayKey ? "present" : "missing");

    if (!razorpayKey) {
      toast.error("Razorpay key is missing. Please verify the Vercel environment variable.");
      return;
    }
...
async function sendPaymentSuccessEmail(response, amount, token) {
  try {
    await axios.post(
      `${API_BASE_URL}/sendPaymentSuccessEmail`,
      {
